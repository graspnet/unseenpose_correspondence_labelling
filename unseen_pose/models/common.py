...
def get_norm(norm_type, n_channels, D, bn_momentum=0.1):
  #   Test No Track Running stats
  if norm_type == NormType.BATCH_NORM:
    return ME.MinkowskiBatchNorm(n_channels, momentum=bn_momentum, track_running_stats=False)
    ...
    return nn.Sequential(
        ...
        ME.MinkowskiBatchNorm(n_channels, momentum=bn_momentum, track_running_stats=False))
  else:
...
# Upd Note. ME v0.5 make ME.RegionType uniterable.
# int_to_region_type = {m.value: m for m in ME.RegionType}
int_to_region_type = {0: ME.RegionType.HYPER_CUBE,
...
def conv(in_planes,
  ...
  # Upd Note. ME v0.5 rename "has_bias" to "bias".
